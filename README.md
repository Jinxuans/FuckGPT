...
FuckGPT 是一个面向本地自用的 ChatGPT/Codex 账号注册、管理、邮箱资源、接码资源和导出流程面板。当前前端侧栏包含五个顶层入口：`总览`、`ChatGPT 账号`、`邮箱资源`、`任务记录`、`设置`。
...
用于管理 ChatGPT 账号及其套餐、检测和 Codex 授权状态：

- 查看 ChatGPT 账号列表、状态、邮箱和账号详情
- 自动注册账号，支持配置注册数量、并发、注册身份和执行方式
- 批量选择、导出和复制账号信息
- 将已选账号以 Codex（默认）或 Sub2Api 格式推送到已配置的远端目标，并查看每个账号的最新推送状态
...
| 邮箱资源   | 本地邮箱账号、别名地址和账号绑定关系管理     |
| 接码服务   | 接码 provider 的新增、启用、默认项和参数配置 |
| 推送目标   | NexusVault 等账号推送目标的地址、密钥和数据格式配置 |
...
本项目使用 AGPL-3.0 许可证。项目基于 `asz798838958/freeAgentIdentity` 继续维护，并保留上游项目所基于的 `lxf746/any-auto-register` 插件化注册框架说明，感谢原作者和上游维护者的开源工作。

## 仓库

- 当前维护仓库：`https://github.com/Jinxuans/FuckGPT`
- 上游仓库：`https://github.com/asz798838958/freeAgentIdentity`
